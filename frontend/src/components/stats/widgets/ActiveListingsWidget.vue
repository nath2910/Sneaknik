...
import { formatNumber, signFmt } from '@/utils/formatters'
...
async function load() {
  ...
  try {
    const [k, s] = await Promise.all([
      StatsServices.kpi('activeListings', props.from, props.to),
      StatsServices.series('activeListings', props.from, props.to, props.bucket),
    ])
    if (id !== req) return
...
const valueText = computed(() => formatNumber(kpi.value.value, { compact: true }))
